...
const getDayDiff = (a: Date, b: Date) =>
  Math.round((getStartOfDay(a).getTime() - getStartOfDay(b).getTime()) / MS_PER_DAY)
const getWeekDiff = (a: Date, b: Date) =>
  Math.round((getStartOfWeek(a).getTime() - getStartOfWeek(b).getTime()) / (7 * MS_PER_DAY))
const getMonthDiff = (a: Date, b: Date) => (a.getFullYear() - b.getFullYear()) * 12 + (a.getMonth() - b.getMonth())

const dateToTimeString = (d: Date) => d.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })
...
export function formatExactDate(date: Date | string | number): string {
  ...
  const offset = target.toLocaleTimeString("en", { timeZoneName: "shortOffset" }).split(" ").pop()
...
export function formatRelativeDate(date: Date | string | number, now: Date = new Date()): string {
  ...
  if (Math.abs(days) < 7) return `${formatRelativeLabel(Math.abs(days), "day", days > 0)} at ${time}`

  const weeks = getWeekDiff(target, now)
  if (Math.abs(weeks) < 5) return formatRelativeLabel(Math.abs(weeks), "week", weeks > 0)

  const months = getMonthDiff(target, now)
  if (Math.abs(months) < 12) return formatRelativeLabel(Math.abs(months), "month", months > 0)
...
export function formatDetailedDuration(date: Date | string | number, now: Date = new Date()): string {
  const target = new Date(date)
  const future = target.getTime() > now.getTime()
  const totalSeconds = Math.abs(Math.round((target.getTime() - now.getTime()) / 1000))

  const { parts } = DURATION_UNITS.reduce(
    ({ remaining, parts }, [unit, secondsPerUnit]) => {
      const amount = Math.floor(remaining / secondsPerUnit)
      const label = amount > 0 ? [`${amount} ${unit}${amount > 1 ? "s" : ""}`] : []
      return { remaining: remaining % secondsPerUnit, parts: [...parts, ...label] }
    },
    { remaining: totalSeconds, parts: [] as string[] },
  )

  const joined = parts.length > 1 ? `${parts.slice(0, -1).join(", ")} and ${parts.at(-1)}` : (parts[0] ?? "less than a second")

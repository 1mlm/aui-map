...
export type CustomTableColumn<T> = {
  ...
  | { type: "tags"; getTags: (item: T) => CustomTableEnumValue[] }
...
export function getColumnExportValue<T>(column: CustomTableColumn<T>, item: T): string {
  if (column.type === "string" || column.type === "copy") return column.getString(item)
  if (column.type === "date") return column.getDate(item)?.toISOString() ?? ""
  if (column.type === "boolean") return column.getBoolean(item) ? "true" : "false"
  if (column.type === "tags")
    return column
      .getTags(item)
      .map((tag) => tag.label)
...
function getSearchableStrings<T>(columns: CustomTableColumn<T>[], item: T): string[] {
  return columns.flatMap((column) => {
    if (column.type === "string") return [column.getString(item)]
    if (column.type === "copy") return column.searchable === false ? [] : [column.getString(item)]
    if (column.type === "enum") {
      const value = column.getValue(item)
      return value !== undefined ? [column.enumOptions[value]?.label ?? ""] : []
    }
    if (column.type === "tags") return column.getTags(item).map((tag) => tag.label)
    return []
  })
}
...
export function CustomTable<T>({
  ...
  const setColumnField = (columnId: string, field: ColumnFilterField, value: string) =>
    setFilterValues({ [getFilterKey(columnId, field)]: value })

  const sort = sortable ? parseSort(sortRaw) : null
  const hasActiveFilterOrSort = Object.values(filterValues).some(Boolean) || sort !== null
  const resetFilterAndSort = () => {
    setFilterValues(Object.fromEntries(Object.keys(filterValues).map((key) => [key, ""])))
    setSortRaw("")
  }
  ...
  const pageCount = paginate ? Math.max(1, Math.ceil(visibleItems.length / PAGE_SIZE)) : 1
  const currentPage = Math.min(Math.max(page, 1), pageCount)
  const paginatedItems = paginate ? visibleItems.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE) : visibleItems
  ...
  const toggleAll = () => setSelectedIds(allSelected ? new Set() : new Set(visibleItems.map(getItemId)))

  const canResetFilterAndSort = (filterable || sortable) && hasActiveFilterOrSort
              ...
              {selectable && (
                <TableHead>
                  <div className="flex justify-center pr-2!">
                    <Checkbox
                      checked={getTriState(selectedIds.size, visibleItems.length)}
              ...
              {columns.map((column) => (
                <TableHead key={column.id}>
                  <CustomTableColumnHeader
                    ...
                    setField={(field: ColumnFilterField, value: string) => setColumnField(column.id, field, value)}
                    sort={sort?.columnId === column.id ? sort.dir : null}
                    onSortChange={(dir: "asc" | "desc" | null) => setSortRaw(dir ? serializeSort({ columnId: column.id, dir }) : "")}
                  />
                </TableHead>
              ))}
            </TableRow>
          ...
          <TableBody>
            {loading &&
              SKELETON_ROW_KEYS.map((key, index) => (
                <TableRow key={key} className={cn(index % 2 === 1 && "bg-foreground/5")}>
                  {selectable && (
                  ...
                  {columns.map((column) => (
                    <TableCell key={column.id} className="border-r border-border/50 last:border-r-0">
                    ...
                    {columns.map((column) => (
                      <TableCell
                        ...
                        className={cn(
                          ...
                          column.type === "string" && column.align === "right" && "text-right",
                          (column.type === "copy" || column.type === "enum" || column.type === "tags" || column.type === "buttons") &&
                            "text-center",
                        )}
                      >
      ...
      {showActionBar && (
        <div className="fixed inset-x-4 bottom-4 flex flex-col items-end gap-2 sm:inset-x-auto sm:bottom-8 sm:right-8 sm:flex-row border border-border bg-sidebar px-4 py-2 rounded-full corner-squircle shadow-[0_0_16px_rgba(0,0,0,0.35)]">
          ...
          {canResetFilterAndSort && (
            <Button variant="outline" className="shadow-lg" onClick={resetFilterAndSort}>
              <Icon icon={BrushCleaningIcon} />
              <span className="hidden sm:inline">Reset filters &amp; sorting</span>
              <span className="sm:hidden">Reset</span>
            </Button>
          )}
          {selectable && <ExtractButton {...{ selectedItems, columns }} filePrefix={exportFilePrefix} />}
        </div>
      )}
    </div>
  )

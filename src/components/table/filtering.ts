...
export type ColumnFilterField = "excluded" | "search" | "min" | "max" | "from" | "to" | "only" | "countMin" | "countMax"

// human readable query param per field, e.g. excluded_role, createdAt_from, tags_count_min
export function getFilterKey(columnId: string, field: ColumnFilterField): string {
...
export function getColumnFilterFields<T>(column: CustomTableColumn<T>): ColumnFilterField[] {
  ...
  if (column.type === "string") return column.filterType === "number" ? ["min", "max"] : ["search"]
  return []
}
...
export function isEnumOptionExcluded(getField: GetFilterField, key: string): boolean {
  return splitList(getField("excluded")).includes(key)
}

export function toggleEnumOption(getField: GetFilterField, key: string): string {
...
function enumMatches(getField: GetFilterField, value: string | undefined): boolean {
...
function dateMatches(from: string, to: string, value: Date | undefined): boolean {
...
function tagsMatches(getField: GetFilterField, tags: CustomTableEnumValue[]): boolean {
  ...
  return tags.some((tag) => [tag.label, ...(tag.keywords ?? [])].some((value) => value.toLowerCase().includes(query)))
}

// whether a column supports filtering/sorting at all (copy/buttons columns are excluded)
export function isColumnFilterableOrSortable<T>(column: CustomTableColumn<T>): boolean {
...
export function getTriState(selectedCount: number, totalCount: number): boolean | "indeterminate" {
  if (selectedCount === 0) return false
  return selectedCount === totalCount ? true : "indeterminate"
}

function getColumnNumber<T>(column: Extract<CustomTableColumn<T>, { type: "string" }>, item: T): number {
  return column.getNumber ? column.getNumber(item) : Number(column.getString(item))
}

export function columnMatchesFilter<T>(column: CustomTableColumn<T>, item: T, getField: GetFilterField): boolean {
  if (column.type === "string") {
    return column.filterType === "number"
      ? numberMatches(getField("min"), getField("max"), getColumnNumber(column, item))
      : textMatches(getField("search"), column.getString(item))
  }
  if (column.type === "date") return dateMatches(getField("from"), getField("to"), column.getDate(item))
  if (column.type === "enum") return enumMatches(getField, column.getValue(item))
  if (column.type === "tags") return tagsMatches(getField, column.getTags(item))
  if (column.type === "boolean") return booleanMatches(getField, column.getBoolean(item))
  return true
}

export function compareColumnValues<T>(column: CustomTableColumn<T>, a: T, b: T): number {
  if (column.type === "string") {
    if (column.filterType === "number") return getColumnNumber(column, a) - getColumnNumber(column, b)
  ...
  if (column.type === "tags") return column.getTags(a).length - column.getTags(b).length
  if (column.type === "boolean") return Number(column.getBoolean(a)) - Number(column.getBoolean(b))
  return 0
}

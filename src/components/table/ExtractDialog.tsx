...
function buildExportRows<T>(items: T[], columns: CustomTableColumn<T>[]) {
  const exportableColumns = columns.filter((column) => column.type !== "buttons")
  return items.map((item) =>
    Object.fromEntries(exportableColumns.map((column) => [column.label, getColumnExportValue(column, item)])),
  )
}

async function downloadAsExcel(rows: Record<string, string>[], filename: string) {
...
function downloadAsCsv(rows: Record<string, string>[], filename: string) {
  ...
  downloadBlob(new Blob([lines.map((line) => line.map(escapeCsvValue).join(",")).join("\n")], { type: "text/csv" }), filename)
}
...
export function ExtractDialog<T>({
  ...
  const handleConfirm = async () => {
    ...
    if (format === "excel") await downloadAsExcel(rows, buildExportFilename(filePrefix, "xlsx"))
    ...
    <Dialog {...{ open, onOpenChange }}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Choose a file format to download the selected rows.</DialogDescription>
        </DialogHeader>

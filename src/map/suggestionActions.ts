...
// the file (if any) is already sitting in Cloudinary by the time this runs — SuggestionForm
...
export async function submitSuggestion(
  ...
  await notifyAdmin(
    kind === "BUG"
      ? "New bug report on AUI Map"
      : "New feature idea for AUI Map",
    `<p>${escapeHtml(trimmed)}</p>${attachment ? `<p>Attachment: <a href="${attachment.url}">${escapeHtml(attachment.fileName)}</a></p>` : ""}`,
  )
}
